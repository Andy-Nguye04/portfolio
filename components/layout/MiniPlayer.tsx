...
export default function MiniPlayer() {
  const { activeLabel } = usePlaylist();

  return (
    <div className="fixed bottom-[60px] left-0 right-0 h-[52px] bg-spotify-dark border-t border-[#2A2A2A] flex items-center justify-between px-4 z-30 md:hidden">
      <div className="flex items-center gap-3">
        {/* Spotify icon */}
        <div className="w-7 h-7 rounded-full bg-spotify-green flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex flex-col">
          <p className="text-white text-xs font-semibold leading-tight">{activeLabel}</p>
          <p className="text-spotify-muted text-[10px] leading-tight">Now Playing</p>
        </div>
      </div>
    </div>
  );
